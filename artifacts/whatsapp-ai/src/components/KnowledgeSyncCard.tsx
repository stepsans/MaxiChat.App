...
import {
  ...
  useUpsertKnowledgeSyncConfig,
  useRunKnowledgeSync,
  useListCredentials,
  useListCredentialSpreadsheets,
  useListCredentialSpreadsheetTabs,
  getGetKnowledgeSyncConfigQueryKey,
...
export default function KnowledgeSyncCard({
  onActiveChange,
}: {
  ...
  const { data: cfgResp, isLoading: cfgLoading } = useGetKnowledgeSyncConfig({
    query: { queryKey: getGetKnowledgeSyncConfigQueryKey(), refetchInterval: 30_000 },
  });
  const cfg: KnowledgeSyncConfig | null = cfgResp?.config ?? null;
  ...
  const saveMut = useUpsertKnowledgeSyncConfig({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getGetKnowledgeSyncConfigQueryKey() });
  ...
  const runMut = useRunKnowledgeSync({
    mutation: {
      onSuccess: (res) => {
          ...
          {cfgLoading ? (
            ...
            <>
              ...
              <div className="md:col-span-2 text-[11px] text-muted-foreground">
                Header tab wajib: <code className="font-mono">type</code>,{" "}
                <code className="font-mono">title</code>,{" "}
                <code className="font-mono">content</code>. Sheet adalah source of truth
                — entry yang hilang dari sheet akan
                <strong className="text-foreground"> dihapus</strong> dari knowledge base
                saat sync.
              </div>
            </>
          )}
