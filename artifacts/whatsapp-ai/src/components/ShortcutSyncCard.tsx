...
import {
  useGetShortcutSyncConfig,
  useUpsertShortcutSyncConfig,
  useRunShortcutSync,
  useListCredentials,
  useListCredentialSpreadsheets,
  useListCredentialSpreadsheetTabs,
  getGetShortcutSyncConfigQueryKey,
...
export default function ShortcutSyncCard({
  onActiveChange,
}: {
  ...
  const { data: cfgResp, isLoading: cfgLoading } = useGetShortcutSyncConfig({
    query: { queryKey: getGetShortcutSyncConfigQueryKey(), refetchInterval: 30_000 },
  });
  const cfg: ShortcutSyncConfig | null = cfgResp?.config ?? null;
  ...
  const saveMut = useUpsertShortcutSyncConfig({
    mutation: {
      onSuccess: () => {
        qc.invalidateQueries({ queryKey: getGetShortcutSyncConfigQueryKey() });
  ...
  const runMut = useRunShortcutSync({
    mutation: {
      onSuccess: (res) => {
  ...
  return (
    <div className="border-b border-border bg-muted/10 rounded-md">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
          ...
          {cfgLoading ? (
            ...
            <>
              ...
              <div className="md:col-span-2 text-[11px] text-muted-foreground">
                Header tab wajib: <code className="font-mono">shortcut</code>,{" "}
                <code className="font-mono">replacement</code>. Sheet adalah source of
                truth — shortcut yang hilang dari sheet akan
                <strong className="text-foreground"> dihapus</strong> saat sync.
              </div>
            </>
          )}
        </div>
